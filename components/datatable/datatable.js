...
var DataTable = (function () {
    ...
    DataTable.prototype.paginateSilently = function (event) {
        this.first = event.first;
        this.rows = event.rows;
        if (this.lazy)
            this.onLazyLoad.emit(this.createLazyLoadMetadata());
            ...
            this.updateDataToRender(this.filteredValue || this.value);
        this.pgtr.rows = this.rows;
        this.pgtr.changePageToFirst();
    };
    DataTable.prototype.setTableMetaInfo = function () {
        if (this.totalRecords == 0) {
            this.pFrom = 0;
            this.pTo = 0;
        }
        else {
            this.pFrom = this.first + 1;
            if (this.pFrom + this.rows - 1 > this.totalRecords)
                this.pTo = this.totalRecords;
            else
                this.pTo = this.pFrom + this.rows - 1;
        }
    };
    ...
    __decorate([
        core_1.ViewChild('pgtr'), 
        __metadata('design:type', Object)
    ], DataTable.prototype, "pgtr", void 0);
    __decorate([
        core_1.Input(), 
        __metadata('design:type', Boolean)
